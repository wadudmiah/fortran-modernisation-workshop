...
\end{verbatim}
Replace the \texttt{<firstname>} and \texttt{<lastname>} with your name :-) The Git commands
...
using the command:
\begin{verbatim}
nagfor =unifyprecision -pp_module=Types_mod -pp_name=DP fd1d_heat_explicit.f90 \
  -o fd1d_heat_explicit.f90_prs
...
\begin{enumerate}
\item change relational operators (\texttt{.LE.} to \texttt{<=})
...
\begin{verbatim}
nagfor =polish -relational=F90+ -kwcase=C -margin=0 -indent=2 \
  -dcolon_in_decls=Insert fd1d_heat_explicit.f90 -o fd1d_heat_explicit.f90_pol
...
\item To compile the remember to add the line \texttt{-I/usr/lib64/nagfor/modules} in your
...
current time step
\item To compile the code, add the line \texttt{-I/usr/lib64/nagfor/modules} - this
