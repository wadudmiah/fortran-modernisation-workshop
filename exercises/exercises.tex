...
\begin{enumerate}
\item Write a Makefile for the Fortran code produced on Day 1 in the same directory as the source
code. After creating the Makefile, add it to git using \texttt{git add Makefile} 
...
specify which files not not version control, e.g. object files, executable files or any file that can
...
\end{tabular}
\end{center}
Then load the file \texttt{doxygen/html/index.html} in any browser to browse the source code documentation.
\begin{enumerate}
