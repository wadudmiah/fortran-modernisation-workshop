...
to \texttt{size( mat(:,:), 1 )} and \texttt{size( mat(:,:), 2 )}, respectively. Declare the argument \texttt{table(:,:)}
...
\item Use the Linux \texttt{nm} to view the symbols in \texttt{libfmw.a} which shows the symbols
  listed in the library. The symbol type field shows 
\end{enumerate}
